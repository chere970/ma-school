...
import { Department, Campus } from '@/types/academic';
import { departmentsApi, CreateDepartmentInput } from '@/lib/api/departments';
...
export default function DepartmentsPage() {
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      if (err instanceof ApiClientError) {
        ...
        setError('Failed to fetch departments. Verify API server status.');
      }
    } finally {
      setLoading(false);
    }
  ...
  const handleDelete = async () => {
    ...
    setSubmitting(true);
    try {
      await departmentsApi.delete(deletingDepartment.id);
  ...
  const filteredDepartments = departments.filter(
    (d) =>
      ...
      (d.campus?.name && d.campus.name.toLowerCase().includes(search.toLowerCase()))
  );
  ...
  return (
    <div className="space-y-6">
      <PageHeader
        title="Departments"
        description="Manage academic departments mapped to university campuses"
        action={
          <button
      ...
      <div className="bg-slate-900/90 border border-slate-800/80 rounded-2xl shadow-xl overflow-hidden">
        <div className="p-4 border-b border-slate-800/80 bg-slate-950/40 flex items-center justify-between">
          <div className="relative w-full max-w-sm">
            ...
            <input
              type="text"
              placeholder="Search department or campus..."
              value={search}
          ...
          <span className="text-xs text-slate-400">
            Total: <strong className="text-slate-200">{filteredDepartments.length}</strong> departments
          </span>
        </div>
              ...
              {loading ? (
                Array.from({ length: 3 }).map((_, idx) => (
                  <tr key={idx} className="animate-pulse">
                    ...
                    <td className="py-4 px-6"><div className="h-4 w-44 bg-slate-800 rounded" /></td>
                ...
                <tr>
                  <td colSpan={5} className="py-12 text-center text-slate-500">
                    <Icons.Department className="w-10 h-10 mx-auto text-slate-700 mb-2" />
                    <p className="font-medium">No departments found</p>
                    <p className="text-[11px]">Click "Add Department" to create one.</p>
                  </td>
                </tr>
              ) : (
                filteredDepartments.map((dept) => (
                  <tr key={dept.id} className="hover:bg-slate-800/40 transition-colors">
                    <td className="py-4 px-6 font-mono font-semibold text-cyan-400">{dept.code}</td>
                    <td className="py-4 px-6 font-medium text-slate-100">{dept.name}</td>
                    <td className="py-4 px-6">
                      <span className="px-2.5 py-1 rounded-lg bg-slate-800 text-slate-200 font-medium text-xs border border-slate-700">
                        {dept.campus?.name || 'Assigned Campus'}
                      </span>
                    </td>
                    <td className="py-4 px-6 text-slate-400 max-w-xs truncate">{dept.description || '—'}</td>
          ...
          <div className="bg-slate-900 border border-slate-800 rounded-2xl max-w-md w-full p-6 shadow-2xl space-y-5">
            <div className="flex items-center justify-between border-b border-slate-800 pb-4">
              <h3 className="text-lg font-bold text-white">
                {isCreateOpen ? 'Create New Department' : 'Edit Department'}
              </h3>
              <button
                onClick={() => {
                  setIsCreateOpen(false);
            ...
            <form onSubmit={isCreateOpen ? handleCreateSubmit : handleEditSubmit} className="space-y-4">
              ...
              <div>
                ...
                <textarea
                  rows={3}
                  placeholder="Optional overview of the department..."
              ...
              <div className="flex items-center justify-end gap-3 pt-4 border-t border-slate-800">
                ...
                >
                  {submitting ? 'Saving...' : isCreateOpen ? 'Create Department' : 'Update Department'}
                </button>
              </div>
            </form>
          </div>
          ...
          <div className="bg-slate-900 border border-slate-800 rounded-2xl max-w-sm w-full p-6 shadow-2xl space-y-4">
            ...
            <div className="text-center">
              ...
              <p className="text-xs text-slate-400 mt-1">
                Are you sure you want to delete <strong className="text-slate-200">{deletingDepartment.name}</strong>?
              </p>
            </div>
